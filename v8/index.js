((() => {
  ...
  function errorText(error) {
    return String(error && (error.stack || error.message) || error).slice(0, 600);
  }

  function recordError(error) {
    diagnostic.errors.push(errorText(error));
    try { console.error("[iOS26Emoji DEBUG] " + errorText(error)); } catch {}
  }
    ...
    for (const row of rows) {
      ...
      if (row && row.type === 1 && row.message && row.message.content) {
        ...
        for (const item of content) {
          if (item && item.type === "emoji") diagnostic.emojiSeen++;
        }
      }
    }
  }
  ...
  function installProbe() {
    ...
    const chatModule = (nativeModules && nativeModules.DCDChatManager) ||
      (typeof window !== "undefined" && window.nativeModuleProxy && window.nativeModuleProxy.DCDChatManager);

    if (chatModule && typeof chatModule.updateRows === "function") {
      diagnostic.bridge = nativeModules && nativeModules.DCDChatManager ?
        "ReactNative.NativeModules.DCDChatManager" : "window.nativeModuleProxy.DCDChatManager";
      const unpatch = patcher.before("updateRows", chatModule, (args) => {
        try {
          diagnostic.updateRowsCalls++;
          const parsed = JSON.parse(args[1]);
          inspectRows(Array.isArray(parsed) ? parsed : parsed && parsed.rows);
    ...
    if (rowManager && rowManager.prototype && typeof rowManager.prototype.generate === "function") {
      ...
      const unpatch = patcher.after("generate", rowManager.prototype, (_args, row) => {
        try { inspectRows([row]); } catch (error) { recordError(error); }
      });
      if (typeof unpatch === "function") unpatches.push(unpatch);
      return;
    }
  ...
  function onUnload() {
    for (const unpatch of unpatches.splice(0)) {
      try { unpatch(); } catch (error) { recordError(error); }
    }
  }
  ...
  function Settings() {
    ...
    return React.createElement(Container, { style: { flex: 1, padding: 16 } },
      React.createElement(ReactNative.Text, { selectable: true, style: { color: "#ffffff", fontSize: 13 } }, JSON.stringify(diagnostic, null, 2))
    );
  }
